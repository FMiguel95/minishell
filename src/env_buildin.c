...
void	env_buildin(char **argv, char **env_copy)
{
	...
	char	*env_var = env_copy[0];	// declare and initialize there is always at least one pointer; the NULL since envp is nullterminated by standard request
	...
	if(argv[0] != NULL && argv[1] != NULL)			// a test number, it will be changed to 1 in original minishell 
	{
