...
//	printf("\nenv_index PWD: %zu\n", env_index);
	...
	new_PWD_str = ft_strjoin("PWD=", getcwd(NULL, 0));
...
//	printf("temp_oldpwd: %s\n", temp_oldpwd);
	new_OLDPWD_str = ft_strjoin("OLDPWD=", extract_value(temp_pwd));
...
//	if (!argv[1][0] || !strcmp(argv[1], "~"))
	else if ((ft_argc(argv) == 1) || !strcmp(argv[1], extract_value(env_cpy[find_key_env_index("HOME", env_cpy, 4)])))
	{
		if (find_key_env_index("HOME", env_cpy, 4) != pointer_array_len(env_cpy))
		{
			...
			exit_status = chdir(extract_value(env_cpy[find_key_env_index("HOME", env_cpy, 4)]));
			if (exit_status)
				ft_putstr_fd("error in changing directory", 2);
	//		str = extract_value(env_cpy[find_key_env_index("HOME", env_cpy, 4)]);
			printf("to be in chdir; %s\n", extract_value(env_cpy[find_key_env_index("HOME", env_cpy, 4)]));
			getcwd(str,100);
			printf("current pwd: %s\n", str);
//			exit_status = 0;
//			return (exit_status);
...
//			return (exit_status);
	...
	else if (!strcmp(argv[1], "-"))
	{
		...
		//	chdir(extract_value(env_cpy[find_key_env_index("OLDPWD", env_cpy, 6)]));
			exit_status = chdir(extract_value(env_cpy[find_key_env_index("OLDPWD", env_cpy, 6)]));
			if (exit_status)
				ft_putstr_fd("error in changing directory\n", 2);
			printf("exit_status: %d\n", exit_status);
	//		str = extract_value(env_cpy[find_key_env_index("HOME", env_cpy, 4)]);
			printf("to be in chdir; %s\n", extract_value(env_cpy[find_key_env_index("OLDPWD", env_cpy, 6)]));
			getcwd(str,100);
			printf("current pwd: %s\n", str);
	//		exit_status = 0;
//			return (exit_status);
